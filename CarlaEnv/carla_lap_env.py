...
class CarlaLapEnv(gym.Env):
    ...
    def __init__(self, host="127.0.0.1", port=2000,
        ...
        # for i in range(10):
        #     print(self.route_waypoints[i][0].transform.location)

        self.agent_index = 0
        # choices = ["cautious", "normal", "aggressive"]
        self.agent = BehaviorAgent(self.vehicle.actor, behavior="normal")
        # self.agent.set_destination(self.vehicle.actor.get_location(),
        #                                    self.route_waypoints[self.agent_index][0].transform.location, clean=True)
    ...
    def reset(self, is_training=True):
        ...
        if is_training:
            ...
            self.current_waypoint_index = self.checkpoint_waypoint_index % len(self.route_waypoints)
        else:
        ...
        self.agent_index = (self.current_waypoint_index + 50) % len(self.route_waypoints)
        self.agent.set_destination(self.vehicle.actor.get_location(),
                                   self.route_waypoints[self.agent_index][0].transform.location, clean=True)
    ...
    def step(self, action, expert=True):
        self.agent.update_information(self.vehicle.actor)
        ...
        if action is not None:
            ...
            self.vehicle.control.throttle = self.vehicle.control.throttle * self.action_smoothing + throttle * (
                        1.0 - self.action_smoothing)
            self.vehicle.control.brake = self.vehicle.control.brake * self.action_smoothing + brake * (
                        1.0 - self.action_smoothing)

        # Tick game

        speed_limit = self.vehicle.actor.get_speed_limit()
        self.agent.get_local_planner().set_speed(speed_limit)
        expert_action = self.agent.run_step()
        if expert:
            self.vehicle.control = expert_action
        output_action = [expert_action.steer, expert_action.throttle, expert_action.brake]
        ...
        frame = self.observation.astype(np.float32) / 255.0
        encoded_state = [frame, np.array([self.vehicle.get_speed()])]
        ...
        for _ in range(len(self.route_waypoints)):
            ...
            if dot > 0.0:  # Did we pass the waypoint?
                ...
                self.current_waypoint_index = waypoint_index % len(self.route_waypoints)
                # DEBUG
                # print(waypoint_index)
                if waypoint_index == self.agent_index - 30:
                    self.agent_index = (self.agent_index + 50) % len(self.route_waypoints)
                    self.agent.set_destination(self.vehicle.actor.get_location(),
                                               self.route_waypoints[self.agent_index][0].transform.location, clean=True)
            else:
                break
        ...
        return encoded_state, output_action, self.last_reward, self.terminal_state, {"closed": self.closed}
